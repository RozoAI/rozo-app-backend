...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authHeader.startsWith('Basic ')) {
      ...
      return new Response('Unauthorized: Invalid authorization format', { status: 401 });
    }
    ...
    console.log(`Received Daimo webhook: ${webhookEvent.type} for payment ${webhookEvent.paymentId}`);

    // Validate required fields
    if (!webhookEvent.type || !webhookEvent.paymentId || !webhookEvent.payment) {
    ...
    if (!validationResult.isValid) {
      ...
      return new Response(`Validation failed: ${validationResult.errors.join(', ')}`, { status: 400 });
    }
    ...
    if (newStatusLevel < currentStatusLevel) {
      console.log(`Ignoring backward status transition from ${currentStatus} to ${newStatus} for payment ${webhookEvent.paymentId}`);
      return new Response('Status transition ignored', { status: 200 });
    }

    // If status is the same, check if this is a duplicate webhook
    if (currentStatus === newStatus) {
      console.log(`Duplicate webhook received for payment ${webhookEvent.paymentId} with status ${newStatus}`);
    ...
    if (webhookEvent.type === 'payment_started' && webhookEvent.payment.source?.txHash) {
 ...
 */
function validatePaymentDetails(order: OrderRecord, webhook: DaimoWebhookEvent): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];

  // Validate destination chain ID
  if (webhook.payment.destination?.chainId !== order.merchant_chain_id) {
    errors.push(`Chain ID mismatch: expected ${order.merchant_chain_id}, got ${webhook.payment.destination?.chainId}`);
  }

  // Validate destination address
  if (webhook.payment.destination?.destinationAddress.toLowerCase() !== order.merchant_address.toLowerCase()) {
    errors.push(`Address mismatch: expected ${order.merchant_address}, got ${webhook.payment.destination?.destinationAddress}`);
  }

  // Validate destination token
  if (webhook.payment.destination?.tokenAddress.toLowerCase() !== order.required_token.toLowerCase()) {
    errors.push(`Token mismatch: expected ${order.required_token}, got ${webhook.payment.destination?.tokenAddress}`);
  }

  // Validate destination amount (allow small rounding differences)
  if (webhook.payment.destination?.amountUnits) {
    const expectedAmount = order.required_amount_usd;
    const actualAmount = parseFloat(webhook.payment.destination.amountUnits);
    const tolerance = 0.00; // 0 cent tolerance
